"""On-chain Aave V3 health factor scanner and liquidation opportunity detector."""
...
HF_SCALE = 10**18
CLOSE_FACTOR = 0.5
FLASH_LOAN_PREMIUM_BPS = 5  # Aave V3 default 0.05%
...
class AaveLiquidationScanner:
    """Detect undercollateralized Aave V3 positions on Base."""
    ...
    async def discover_borrowers(self, limit: int = 500) -> list[str]:
        """Merge cached borrowers, subgraph users, and recent on-chain Borrow events."""
        borrowers: set[str] = set()
        if self.settings.borrower_cache_path.exists():
            cached = json.loads(self.settings.borrower_cache_path.read_text())
        ...
        event_users = await self._scan_recent_borrow_events(blocks=10_000)
        borrowers.update(event_users)

        ordered = sorted(borrowers)
        self.settings.borrower_cache_path.parent.mkdir(parents=True, exist_ok=True)
        self.settings.borrower_cache_path.write_text(
            json.dumps(
                {
    ...
    async def _fetch_subgraph_borrowers(self, limit: int) -> list[str]:
        ...
        try:
            ...
            logger.warning("Subgraph borrower fetch failed: %s", exc)
            return []
    ...
    async def _scan_recent_borrow_events(self, blocks: int = 10_000) -> list[str]:
        ...
        while start <= latest:
            ...
            try:
                ...
                logger.warning("Borrow chunk %s-%s failed: %s", start, end, exc)
            start = end + 1
    ...
    async def _evaluate_user(self, user: str) -> LiquidationTarget | None:
        ...
        flash_fee_usd = (flash_fee / (10**debt_decimals)) * 1.0
    ...
    async def _best_liquidation_pair(
        ...
        for token_addr, meta in self._reserve_meta.items():
            ...
            if usage_as_collateral and _a_token_balance > 0 and best is not None:
                debt_asset = best[1]
                if debt_asset != asset:

...
describe("createCdpHonoMiddleware", () => {
  ...
  it("uses custom schemes when provided", () => {
    ...
    createCdpHonoMiddleware({ routes: ROUTES, schemes: customSchemes });
  ...
  it("passes paywallConfig through", () => {
    ...
    createCdpHonoMiddleware({ routes: ROUTES, paywallConfig });
    expect(paymentMiddlewareFromConfig).toHaveBeenCalledWith(
      ROUTES,
      expect.anything(),
      expect.anything(),
  ...
  it("returns the middleware function from paymentMiddlewareFromConfig", () => {
    const result = createCdpHonoMiddleware({ routes: ROUTES });

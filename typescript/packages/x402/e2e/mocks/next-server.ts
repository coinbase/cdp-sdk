 ...
 * "next/server" fails to resolve under Node.js strict-ESM resolution when
 * @x402/next is loaded as a pre-compiled ESM module. This shim is aliased
 * to "next/server" via vitest's resolve.alias (applied to @x402/next via
 * deps.inline) so the import succeeds without installing the full Next.js
 * dev server.
 *
 * Only implements the surface area that @x402/next actually uses at runtime:

/**
 * End-to-end tests for the Hono CDP middleware (createCdpHonoMiddleware).
 *
 * Validates that a resource server built with @coinbase/x402-hono correctly
...
import {
  startHonoResourceServer,
  HONO_RESOURCE_SERVER_URL,
  PROTECTED_PATH,
} from "../helpers/resource-server-hono.js";
...
describeWithCredentials("Hono middleware x402 E2E", () => {
  ...
  beforeAll(async () => {
    ...
    server = await startHonoResourceServer(payToAccount.address as `0x${string}`);
  });
  ...
  it("should return 200 for the health endpoint", async () => {
    const response = await fetch(`${HONO_RESOURCE_SERVER_URL}/health`);
    expect(response.status).toBe(200);
  });

  it("should return 402 for an unauthenticated request to the protected endpoint", async () => {
    const response = await fetch(`${HONO_RESOURCE_SERVER_URL}${PROTECTED_PATH}`);
    expect(response.status).toBe(402);
  });

  it("should complete a real x402 payment on Base Sepolia via Hono middleware", async () => {
    const wrappedFetch = wrapFetchWithPayment(fetch, result.client);
    const response = await wrappedFetch(`${HONO_RESOURCE_SERVER_URL}${PROTECTED_PATH}`);

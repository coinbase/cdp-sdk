...
vi.mock("@x402/fetch", () => ({
  wrapFetchWithPayment: vi.fn().mockImplementation((fetchFn) => fetchFn),
}));
...
vi.mock("../../../src/core/wallets/evm-signer.js", () => ({
...
vi.mock("../../../src/core/wallets/scw-signer.js", () => ({
...
vi.mock("../../../src/core/wallets/svm-signer.js", () => ({
...
import { CdpX402Client, createCdpX402Client } from "../../../src/core/client/index.js";
import { CdpClient } from "@coinbase/cdp-sdk";
import { registerExactEvmScheme } from "@x402/evm/exact/client";
import { UptoEvmScheme } from "@x402/evm/upto/client";
import { registerExactSvmScheme } from "@x402/svm/exact/client";
import { fromCdpEvmAccount } from "../../../src/core/wallets/evm-signer.js";
import { fromCdpSmartWallet } from "../../../src/core/wallets/scw-signer.js";
...
describe("client", () => {
  ...
  beforeEach(() => {
    ...
    // Clear wallet-selection env vars that may have been loaded from a .env file
    // so unit tests always start from a known state.
  ...
  // -------------------------------------------------------------------------
  // createCdpX402Client — cdp-eoa (default)
  // -------------------------------------------------------------------------
  ...
  describe("createCdpX402Client (cdp-smart)", () => {
    ...
    it("recovers from duplicate-owner error by paging through all smart accounts", async () => {
      const duplicateOwnerError = new Error(
        "Multiple smart wallets with the same owner are not allowed",
      );
      (CdpClient as ReturnType<typeof vi.fn>).mockImplementationOnce(() => ({
        evm: {
          ...
          getOrCreateSmartAccount: vi.fn().mockRejectedValue(duplicateOwnerError),
          listSmartAccounts: vi
            .fn()
            // First page has no match, second page has the existing SCW.
            .mockResolvedValueOnce({
              accounts: [{ address: "0xother", owners: ["0xunrelated"] }],
              nextPageToken: "tok1",
            })
            .mockResolvedValueOnce({
              accounts: [{ address: MOCK_SCW_ADDRESS, owners: [MOCK_EVM_ADDRESS] }],
              nextPageToken: undefined,
            }),
          getSmartAccount: vi.fn().mockResolvedValue({
      ...
      expect(result.evmAddress).toBe(MOCK_SCW_ADDRESS);
      const cdpInstance = (CdpClient as ReturnType<typeof vi.fn>).mock.results[0].value;
      expect(cdpInstance.evm.listSmartAccounts).toHaveBeenCalledTimes(2);
      expect(cdpInstance.evm.getSmartAccount).toHaveBeenCalledWith(
        expect.objectContaining({ address: MOCK_SCW_ADDRESS }),
      );
    });
  });
  ...
  describe("CdpX402Client with spendControls", () => {
    ...
    it("registers a policy and before/after hooks when spendControls is set", async () => {
      ...
      await client.createPaymentPayload(mockPaymentRequired as never);
      // The mock client tracks each registration on the instance.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  describe("pre-flight balance check", () => {
    ...
    it("registers balance check before spendControls hooks (so it short-circuits first)", async () => {
      ...
      const x402Mock = (await import("@x402/core/client")).x402Client as any;
      const lastInstance = x402Mock.mock.instances[x402Mock.mock.instances.length - 1];
      // 1 balance check + 1 spend control = 2 before-hooks, with balance check first.
...
vi.mock("../../../src/core/guardrails/wrap-fetch.js", () => ({
...
describe("CdpX402Client.wrapFetch()", () => {
  it("calls wrapFetchWithPayment with globalThis.fetch when no argument is given", async () => {
    const { wrapFetchWithPayment } = await import("../../../src/core/guardrails/wrap-fetch.js");
    const { CdpX402Client } = await import("../../../src/core/client/index.js");
...
describe("createCdpX402Client", () => {
  ...
  it("returns a client that works with wrapFetchWithPayment", async () => {
    const { wrapFetchWithPayment } = await import("../../../src/core/guardrails/wrap-fetch.js");
    const { createCdpX402Client } = await import("../../../src/core/client/index.js");

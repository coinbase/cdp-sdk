...
} from "../../../src/core/client/balance-check.js";
...
describe("createBalanceCheckHook", () => {
  ...
  it("treats EVM asset comparison as case-insensitive", async () => {
    // Simulate CDP returning a mixed-case (EIP-55 checksummed) address while
    // the request supplies the same address in all-lowercase — both should
    // normalize to the same value.  We keep the `0x` prefix lowercase because
    // that is what real CDP SDK responses produce; only the hex digits vary.
    const cdpClient = buildCdpClient({
      evmBalances: [
        {
          // USDC_BASE_SEPOLIA already has mixed-case hex digits (e.g. "0x036CbD…")
          // which represents the checksummed form the CDP SDK may return.
    ...
    await expect(
      hook(
        ctx({
          network: "eip155:84532",
          // Request supplies all-lowercase; hook must still match.
  ...
  it("raises InsufficientFundsError when asset is absent from wallet balances", async () => {
    // Asset not present → treated as zero balance, not a silent skip.
    const cdpClient = buildCdpClient({
      evmBalances: [
        {

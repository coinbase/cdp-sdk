...
import type { CdpResourceServerConfig, CdpPaymentScheme } from "../../../src/core/server/index.js";
import {
...
// Mocks
//
// x402HTTPResourceServer is the base class CdpResourceServer extends. To make
// `extends` work in tests we mock it as a regular constructor function (not an
// arrow function) so `this` inside the mock refers to the new instance. The
// mock sets `initialize` on `this` so tests can inspect it, and does NOT return
// a replacement object — which means super() in CdpResourceServer properly
// receives the derived-class `this`.
// ---------------------------------------------------------------------------
...
vi.mock("@x402/core/server", () => ({
  ...
  // Use a regular `function` (not arrow) so `this` is the new instance.
  // Storing initialize on `this` means each CdpResourceServer instance gets it.
...
vi.mock("../../../src/core/facilitator/index.js", () => ({
  createCdpFacilitatorClient: vi.fn().mockReturnValue(mockFacilitatorClient),
}));

vi.mock("../../../src/core/wallets/provision.js", () => ({
...
describe("createCdpResourceServer", () => {
  ...
  describe("initialization", () => {
    it("creates an x402ResourceServer with the CDP facilitator client", async () => {
      ...
      const { createCdpFacilitatorClient } = await import("../../../src/core/facilitator/index.js");
    ...
    it("passes credentials to provisionCdpAccounts", async () => {
      const { provisionCdpAccounts } = await import("../../../src/core/wallets/provision.js");
    ...
    it("exposes resolvedRoutes with resolved payTo and extensions", async () => {
      ...
      // resolvedRoutes should contain the resolved route keys from SIMPLE_ROUTES
...
// ---------------------------------------------------------------------------
// Automatic gas-sponsoring extension injection
// ---------------------------------------------------------------------------
...
describe("CdpResourceServer auto-injects gas-sponsoring extensions", () => {
  ...
  it("user-provided bazaar declaration overrides the auto-generated one", async () => {
    ...
    // Gas sponsoring still present
    expect(ext[CDP_EXTENSION_GAS_SPONSORING_EIP2612]).toBeDefined();
    // User bazaar declaration wins

...
Bumped the `@x402/core`, `@x402/evm`, `@x402/extensions`, and `@x402/svm` peer dependencies to `^2.24.0`. `CdpX402Client` now supports the `upto` scheme on Solana when explicitly enabled via `networkSchemes` and registers the `authCapture` scheme by default on Base, alongside `exact` and `upto`.

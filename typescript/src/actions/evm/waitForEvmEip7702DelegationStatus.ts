...
/**
 * Polls getEvmEip7702DelegationOperationStatus until the status is COMPLETED or FAILED, or a timeout occurs.
 *
 * @example
 * ```ts
...
export async function waitForEvmEip7702DelegationOperationStatus(
  ...
  const reload = async () => {
    return client.getEvmEip7702DelegationOperationStatus(delegationOperationId);
  };

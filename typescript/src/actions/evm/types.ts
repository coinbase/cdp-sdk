...
export type SmartAccountActions = Actions & {
   ...
   *
   * This method handles both requirements automatically, making it safe and convenient for
   * developers to sign EIP-712 messages with CDP Smart Wallets.
   *
   * @param {SmartAccountSignAndWrapTypedDataOptions} options - Configuration options for signing.
   * @param {bigint} options.chainId - The chain ID for the signature (used for replay protection).
   * @param {EIP712Message} options.typedData - The EIP-712 typed data message to sign.
   * @param {bigint} [options.ownerIndex] - The index of the owner to sign with (defaults to 0).
   * @param {string} [options.idempotencyKey] - Optional idempotency key for the signing request.
   *
   * @returns A promise that resolves to the signature that can be used with smart contracts.
   *
   * @example
   * ```ts
   * const signature = await smartAccount.signAndWrapTypedData({
   *   chainId: 1n,
   *   typedData: {
   *     domain: {
   *       name: "Permit2",
   ...
   * });
   *
   * // Use signature.signature for smart contract calls
   * console.log(`Signature: ${signature.signature}`);
   * ```
   */
  signAndWrapTypedData(

...
class TestProvisionCdpAccounts:
    async def test_closes_client_when_provisioning_fails(self) -> None:
        ...
        with (
            patch(
                "cdp_x402.core.wallets.provision.resolve_credentials",
                return_value=_resolved_credentials(),
            ),
            patch("cdp_x402.core.wallets.provision.CdpClient", return_value=cdp_client),
        ):
    ...
    async def test_smart_wallet_re_raises_when_search_finds_nothing(self) -> None:
        ...
        """
        original_exc = RuntimeError("Multiple smart wallets with the same owner")
        ...
        with (
            ...
            with pytest.raises(RuntimeError, match="already has a smart wallet"):

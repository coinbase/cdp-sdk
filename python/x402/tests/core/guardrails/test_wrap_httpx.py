...
class TestIsSettled:
    ...
    def test_non_2xx_with_success_header_is_settled(self) -> None:
        # The settlement header is authoritative: a server can confirm on-chain
        # settlement while returning a non-2xx application response.
...
class TestCdpSettlementAwareTransport:
    ...
    async def _run_transport(
        ...
        # Build the base64-encoded PAYMENT-REQUIRED header for the 402 response.
    ...
    async def test_confirm_called_on_success(self) -> None:
        ...
        # Entry stays in ledger after confirm.
    ...
    async def test_confirm_called_when_non_2xx_but_success_true_header(self) -> None:
        ...
        client, resolved, _ = self._make_client_with_controls()
        # 403 response but the server explicitly confirms on-chain settlement.

...
class TestCreateCDPx402ClientEoa:
    async def test_returns_client_result_with_evm_address(
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", return_value=cdp_mock),
            patch("cdp.x402.core.client.from_cdp_evm_account", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client"),
        ):
...
class TestCreateCDPx402ClientScw:
    async def test_provisions_owner_eoa_and_smart_account(
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", return_value=cdp_mock),
            patch("cdp.x402.core.client.from_cdp_smart_wallet", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client"),
        ):
    ...
    async def test_uses_from_cdp_smart_wallet_for_signer(
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", return_value=cdp_mock),
            patch(
                "cdp.x402.core.client.from_cdp_smart_wallet", return_value=MagicMock()
            ) as mock_scw,
            patch("cdp.x402.core.client.register_exact_evm_client"),
        ):
            await create_cdp_x402_client(scw_config)
...
class TestCDPx402Client:
    ...
    def test_forwards_register_methods_to_inner_client(self) -> None:
        ...
        with patch("cdp.x402.core.client.x402Client", return_value=inner_mock):
            client = CDPx402Client()
    ...
    async def test_initializes_lazily_on_create_payment_payload(
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", return_value=cdp_mock) as mock_cdp_cls,
            patch("cdp.x402.core.client.from_cdp_evm_account", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client"),
            patch("cdp.x402.core.client.x402Client", return_value=inner_mock),
        ):
            client = CDPx402Client()
    ...
    async def test_retries_after_transient_failure(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", side_effect=side_effect),
            patch("cdp.x402.core.client.from_cdp_evm_account", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client"),
            patch("cdp.x402.core.client.x402Client", return_value=inner_mock),
        ):
...
class TestCreateCDPx402ClientSpendControls:
    async def test_applies_spend_controls_when_provided(
        ...
        with (
            ...
            patch("cdp.x402.core.client.from_cdp_evm_account", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client"),
            patch("cdp.x402.core.guardrails.apply.apply_spend_controls") as mock_apply,
        ):
...
class TestCDPx402ClientSpendControls:
    ...
    async def test_wires_spend_controls_after_scheme_registration(
        ...
        with (
            patch("cdp.x402.core.client.CdpClient", return_value=cdp_mock),
            patch("cdp.x402.core.client.from_cdp_evm_account", return_value=MagicMock()),
            patch("cdp.x402.core.client.register_exact_evm_client", side_effect=mock_register),
            patch("cdp.x402.core.guardrails.apply.apply_spend_controls", side_effect=mock_apply),
            patch("cdp.x402.core.client.x402Client", return_value=inner_mock),
        ):
            config = CDPx402ClientConfig(spend_controls=MOCK_SPEND_CONTROLS)
...
class TestAsyncClientMethod:
    ...
    def test_returns_httpx_async_client(self) -> None:
        ...
        client = CDPx402Client()
        with patch(
            "cdp.x402.core.guardrails.wrap_httpx.wrap_httpx_with_payment",

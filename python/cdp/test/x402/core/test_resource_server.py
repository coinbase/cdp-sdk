"""Unit tests for cdp.x402.core.resource_server.
...
from cdp.x402.core.wallets.config import WalletConfig
from cdp.x402.core.wallets.provision import CDPAccountProvisionResult
...
def _patch_create_flow(
    *,
    ...
    captures_patch_targets = [
        patch(
            "cdp.x402.core.resource_server.create_cdp_facilitator_client",
            captured["facilitator_factory_mock"],
        ),
        patch(
            "cdp.x402.core.resource_server.x402ResourceServer",
            return_value=resource_server_stub,
        ),
        patch(
            "cdp.x402.core.resource_server.provision_cdp_accounts",
            provision_mock,
        ),
        patch(
            "cdp.x402.core.resource_server.x402HTTPResourceServer.__init__",
            _init_spy,
        ),
        patch(
            "cdp.x402.core.resource_server.x402HTTPResourceServer.initialize",
            init_mock,
        ),
    ]
...
class TestDefaultsAndHelpers:
    ...
    def test_get_cdp_extension_registrations_falls_back_on_import_error(self) -> None:
        """ImportError from the upstream bazaar module triggers the pass-through fallback."""
        with patch(
            "cdp.x402.core.extensions.bazaar_resource_server_extension",
            side_effect=ImportError("bazaar not installed"),
            create=True,
        ):
            with patch(
                "cdp.x402.core.extensions.__builtins__",
                side_effect=None,
                create=True,
            ):
                pass
    ...
    def test_get_cdp_extension_registrations_does_not_swallow_non_import_errors(
        ...
        """Non-ImportError exceptions from the bazaar module must propagate, not be silenced."""
        import builtins
...
# ---------------------------------------------------------------------------
# Receiver-wallet env isolation regression (H3)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RouteConfig dataclass support (M4)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RouteConfig field preservation — no silent data loss
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Wallet type isolation: server must not be poisoned by payer CDP_WALLET_TYPE
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Empty-routes check before provisioning (M3)
# ---------------------------------------------------------------------------

...
class InitiateEmailAuthenticationNextStep(BaseModel):
    """
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="The type of the next step.")
    url: StrictStr = Field(description="The URL at which to send the verification code.")
    __properties: ClassVar[List[str]] = ["type", "url"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['verify-otp']):
            raise ValueError("must be one of enum values ('verify-otp')")
        return value

...
class EIP3009Payload(BaseModel):
    """
    ...
    data: EIP712Message = Field(description="EIP-712 typed data for a TransferWithAuthorization. Pass to `eth_signTypedData_v4`.")
    __properties: ClassVar[List[str]] = ["payloadId", "type", "data"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['eip3009']):
            raise ValueError("must be one of enum values ('eip3009')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of EIP3009Payload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of EIP3009Payload from a dict"""
        if obj is None:
            return None

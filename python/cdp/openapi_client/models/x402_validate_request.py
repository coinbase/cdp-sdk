...
class X402ValidateRequest(BaseModel):
    ...
    """ # noqa: E501
    resource: Annotated[str, Field(min_length=12, strict=True, max_length=2048)] = Field(description="HTTPS URL of the x402 endpoint to validate. Must use the `https://` scheme.")
    method: Optional[StrictStr] = Field(default='GET', description="The HTTP method used to probe the endpoint. Only GET and POST are supported; other verbs are intentionally rejected because x402 resources are expected to respond to these methods.")
    __properties: ClassVar[List[str]] = ["resource", "method"]

...
async def quote_fund(
    ...
    card_payment_method = next((method for method in payment_methods if method.type == "card" and "source" in method.actions), None)
    ...
    create_payment_transfer_quote_request = CreatePaymentTransferQuoteRequest(
        ...
        target=TransferTarget(CryptoRailAddress(symbol=quote_fund_options.token, network=quote_fund_options.network, address=address)),
        amount=amount,

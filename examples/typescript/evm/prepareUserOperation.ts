...
async function main() {
  ...
  const smartAccount = await toCoinbaseSmartAccount({
    client: publicClient,
    owners: [ownerAccount, SPEND_PERMISSION_MANAGER_ADDRESS],
  });
  ...
  try {
    // The smart account's signTypedData should handle all the complexity
    ...
    console.log("Step 7: Preparing approveWithSignature transaction...");
    ...
    console.log(`Function selector: ${approveData.slice(0, 10)}`);
    ...
    if (response.toLowerCase() === 'y') {
      ...
      // const hash = await spenderWalletClient.sendTransaction({
      ...
      const bundlerClient = createBundlerClient({
        transport: http('https://cloud-api-dev.cbhq.net/rpc/v1/base-sepolia/tBHPlkIaAWn4sS38wD67uo0aSC9mC1tf'),
        account: smartAccount,
      });

// Usage: pnpm tsx evm/smartAccount.networkHoistingSwap.ts
...
console.log(`Note: This example is using ${NETWORK} network with smart accounts. Make sure you have funds available.`);
  ...
  // Create owner account for the smart account
  const ownerAccount = await cdp.evm.getOrCreateAccount({ 
    name: "SmartAccountNetworkHoistingOwner" 
  });
  console.log(`\nOwner account created: ${ownerAccount.address}`);

  // Create base smart account
  const baseSmartAccount = await cdp.evm.getOrCreateSmartAccount({
    name: "NetworkHoistingSwapDemo",
    owner: ownerAccount
  });
  console.log(`Base smart account created: ${baseSmartAccount.address}`);
   ...
   */
   ...
   try {
     ...
     console.log(`\nStep 4: Evaluating additional smart account quote conditions...`);
     ...
     console.log(`\nStep 5: Execute smart account swap (DEMO ONLY)`);
     ...
     if (isGoodRate && isAcceptableSlippage) {
       ...
       const receipt = await networkSmartAccount.waitForUserOperation({
         userOpHash: result.userOperationHash,
       });
       
       console.log("\nSmart Account Swap Confirmed!");
       console.log(`User Operation Status: ${receipt.status}`);
       console.log(`Transaction Hash: ${receipt.transactionHash}`);
       console.log(`Block number: ${receipt.blockNumber}`);
       console.log(`Gas used: ${receipt.gasUsed}`);
     } else {
       console.log(`❌ Conditions not met, skipping smart account execution`);
       console.log(`Rate acceptable: ${isGoodRate ? '✅' : '❌'}`);
       console.log(`Slippage acceptable: ${isAcceptableSlippage ? '✅' : '❌'}`);
     }
     */

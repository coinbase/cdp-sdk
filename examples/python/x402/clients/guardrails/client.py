...
Demonstrates how to use ``cdp_x402`` with a CDP Server Wallet and spend
...
async def main() -> None:
    spend_controls = SpendControls(
        ...
        # A raising callback is silently caught and will never block a payment.
    ...
    # create_cdp_x402_client eagerly provisions a CDP Server Wallet and wires
    # the spend controls onto the returned x402Client.  Credentials are read
    # from environment variables unless overridden via CdpX402ClientConfig.
    result = await create_cdp_x402_client(
        CdpX402ClientConfig(spend_controls=spend_controls)
    )
    ...
    try:
        ...
        # The httpx x402 transport wraps SpendControlError in PaymentError.
        # Walk the cause chain to surface the original error code when available.
        cause = exc
